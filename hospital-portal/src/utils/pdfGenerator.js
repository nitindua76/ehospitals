...
export const generateHospitalPDF = async (form, refId, attachments = {}, token, API_URL, onProgress) => {
  ...
  console.log('📦 pdfGenerator received attachments:', JSON.stringify(attachments, null, 2));
  console.log('📝 pdfGenerator received form name:', form?.name);
  ...
  try {
    ...
    console.log(`🔍 Total keys in attachments object: ${attachmentKeys.length}`, attachmentKeys);
    ...
    console.log(`📑 PDF Merge: Processing ${validAttachments.length} valid attachments for ${form.name}`);
